...
class OffbPosCtl:
	...
	tag_fixed_x = self.tag_pt_x
	    ...
	    print(err_x)
	    #print(err_y)
            x_change = -((err_x*self.KP*2) - (((err_x - self.x_prev_error)*70)* self.KD)) #+ (self.x_sum_error * self.KI*0.1))
            y_change = -((err_y*self.KP*2) - (((err_y - self.y_prev_error)*70)* self.KD)) #+ (self.y_sum_error * self.KI*0.1))
	    #print(x_change)
            #print(y_change)
